...
## 1. 서브쿼리란? :
- 하나의 SQL 문에 포함되어 있는 또 다른 SQL문입니다.
- 가장 일반적으로는 WHERE절에서 사용하고, 위치에 따라 SELECT, FROM, WHERE절에서 사용합니다.

* * *

## 2. SELECT절 서브쿼리 :
- 스칼라 서브쿼리 (Scalar Subqueries)​라고 불리며, SELECT 절 안에 서브쿼리가 들어있습니다.
...
## 3. FROM절 서브쿼리 :
- 인라인뷰 (Inline Views)라고 불리며, FROM 절 안에 서브쿼리가 들어있습니다.
- 반드시 서브쿼리 결과는 하나의 테이블로 리턴되어야합니다.
...
## 4, WHERE절 서브쿼리 :
- 중첩 서브쿼리 (Nested Subqueries)라고 불리며, WHERE 절 안에 서브쿼리가 들어있습니다.
