...
## 글로벌 키워드 :
### include :
- 파이프라인 설정을 여러 파일로 분할해 관리하는 기능으로 외부 yaml 파일의 확장자가 반드시 .yml 혹은 .yaml 이어야 합니다.
...
- YAML 파일을 포함하는 가장 간단하고 직접적인 방법인 로컬 파일을 포함하는 방법은 아래와 같습니다.
...
### variables :
- 환경변수를 설정하는데 사용
- 이프라인의 글로벌 수준, 특정 단계(stage), Job 등 다양한 레벨에서 변수를 설정할 수 있어, 파이프라인 실행 시 유용한 정보를 저장하거나 커스텀 값을 사용할 수 있습니다.
...
- Gitlab UI 화면에서 VARIABLE 등록방법은 아래 그림과 같습니다.
...
## 잡 키워드 :
### artifats :
- 빌드나 테스트 과정에서 생성된 파일을 저장하고 공유합니다.
...
### needs :
- 특정 Job이 다른 Job의 실행 결과에 의존할 때 사용합니다.
- 다른 stage나 Job이 먼저 실행되도록 명시적으로 설정할 수 있습니다.
...
> 
> 만약 이러한 변수나 경로 설정이 없다면, 정리 작업이 실패하거나 리소스 삭제가 누락될 수 있습니다.
> 
{: .prompt-tip}
...
- 여러 Job에서 동일한 설정을 반복하지 않고, 공통된 Job 템플릿을 정의한 후 이를 상속하여 재사용할 수 있습니다.
...
> extends 사용 시의 규칙
>
> **점(.)으로 시작하는 템플릿**: 보통 공통 템플릿은 이름 앞에 .을 붙여 표시하는데 이 점(.)으로 시작하는 이름은 GitLab에서 직접 실행되지 않는 템플릿으로 간주합니다.
> 
{: .prompt-info}
...
- 실패해도 파이프라인을 멈추지 않고 다음 Job을 계속 실행할 수 있습니다.
...
> 
> **디버깅 어려움**: 여러 Job에 대해 allow_failure를 활성화하면, 파이프라인 문제를 추적하고 해결하기 어려울 수 있습니다.
{: .prompt-warning}
...
> 사용 이유
>
> - 불필요한 파일 전송을 줄여서 파이프라인의 성능을 최적화할 수 있습니다.
> 
...
> tags 주의사항
>
> **Runner 태그 설정 확인:** 지정한 태그가 실제 Runner 설정에 있는지 확인해야합니다.
>
> **공백 주의:** 태그 이름은 공백 없이 작성해야합니다.
> 
{: .prompt-warning}
