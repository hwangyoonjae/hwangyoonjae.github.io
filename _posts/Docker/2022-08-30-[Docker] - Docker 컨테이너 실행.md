...
```
![텍스트](/assets/img/post/docker/docker%20container%20ID%20%EC%B6%9C%EB%A0%A5%20%ED%99%94%EB%A9%B4.PNG)

| 옵션 | 의미 |
|:-----:|-----|
| **-d** | 실행 화면을 보지 않고 백그라운드로 컨테이너를 실행시킨다는 옵션 |
| **-p** | <호스트포트>:<컨테이너포트> |
| **-it** | 표준 입력을 열어두고(-i), 가상 터미널을 열어(-t) 키보드의 입력을 표준 입력으로 전달할 수 있도록합니다. |
| **-v** | 볼륨 마운트 이용 시 사용합니다. |
| **--name** | 컨테이너의 이름을 지정합니다. |
| **--rm** | 컨테이너를 stop 시키는 동시에 삭제까지 한번에 실행합니다.|
...
### 1.4 실행중인 도커 컨테이너 (Docker Container) 목록 확인 :
```bash
$ docker container ls
또는
$ docker ps
...
### 1.5 실행중인 도커 컨테이너 (Docker Container) 명령어 전달 :
...
- 아래와 같이 명령어 사용을 위해 컨테이너 內 설치를 진행합니다.
