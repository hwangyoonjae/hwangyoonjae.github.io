...
- Master 서버의 Binary로그를 Slave서버가 Relay로그에 저장해서 복제하는 방식입니다.
...
## 2. MariaDB Replication 실습하기 :
- Maria DB가 설치된 후 진행되는 실습입니다.
...
### 3.2 Master Server MariaDB 재시작 및 접속하기 :
```bash
# MariaDB 재시작
...
### 3.4 Master Server binay log 확인하기 :
```sql
> show master status;
```
![텍스트](/assets/img/post/DB/master%20%EC%84%9C%EB%B2%84%20%EC%83%81%ED%83%9C%20%ED%99%95%EC%9D%B8.PNG)
...
### 4.2 Slave Server MariaDB 재시작 및 접속하기 :
```bash
# MariaDB 재시작
$ service mariadb restart
또는
...
### 4.4 Slave Server 실행 및 상태 확인하기 :
```sql
> start slave;

> show slave status;
```
![mariadb slave server 동작확인 화면](/assets/img/post/DB/mariadb%20slave%20server%20%EB%8F%99%EC%9E%91%ED%99%95%EC%9D%B8%20%ED%99%94%EB%A9%B4.PNG)
...
```
![mariadb masterdb에 db생성](/assets/img/post/DB/mariadb%20masterdb%EC%97%90%20db%EC%83%9D%EC%84%B1.PNG)
...
> create table test(
    ...
    -> );
```
![mariadb masterdb에 테이블 생성](/assets/img/post/DB/mariadb%20masterdb%EC%97%90%20%ED%85%8C%EC%9D%B4%EB%B8%94%20%EC%83%9D%EC%84%B1.PNG)
...
> select * from test;
```
![mariadb masterdb에 데이터 삽입과 조회](/assets/img/post/DB/mariadb%20masterdb%EC%97%90%20%EB%8D%B0%EC%9D%B4%ED%84%B0%20%EC%82%BD%EC%9E%85%EA%B3%BC%20%EC%A1%B0%ED%9A%8C.PNG)
...
> select * from test;
```
![mariadb slavedb sync 확인](/assets/img/post/DB/mariadb%20slavedb%20sync%20%ED%99%95%EC%9D%B8.PNG)
